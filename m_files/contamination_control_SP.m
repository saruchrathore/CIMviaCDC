...
nodal_demand30=epa30.d(dc_index,:);

dc30=-nodal_demand30;

nodal_pressure30=epa30.p;

nodal_head30=epa30.head;

nodal_quality30=epa30.nodequality(dc_index,:);

link_quality30=epa30.linkquality;      % Link contamination in open loop simulation

%% Delete demand pattern
...
while (tstep>0)
    ...
    dcCon_all(:,i)=dc_con;
    pnHeadCon_all(:,i)=pSupHead_con/0.098;     % [bar --> m]
    p_barCon_all(:,i)=pbar_con/0.098;       % [bar --> m]
...
nodal_demand_con=epacon.d(dc_index,:);

nodal_pressure_con=epacon.p;

nodal_head_con=epacon.head;

nodal_quality_con=epacon.nodequality(dc_index,:);
